...
{
    [Serializable]
    public class TicketInfo : ReadOnlyBase<TicketInfo>
    {
        ...
        public static readonly PropertyInfo<string> NamaCustomerProperty = RegisterProperty<string>(c => c.NamaCustomer);
        public string NamaCustomer
        {
            get { return GetProperty(NamaCustomerProperty); }
            private set { LoadProperty(NamaCustomerProperty, value); }
        }
        ...
        public static TicketInfo GetTicketInfo(long idticket)
        {
            return DataPortal.Fetch<TicketInfo>(idticket);
        }

        public static TicketInfo GetTicketInfo(SafeDataReader dr)
        {
            return DataPortal.Fetch<TicketInfo>(dr);
        }

        public static TicketInfo GetTicketInfo(string email)
        {
            TicketInfo obj = DataPortal.Fetch<TicketInfo>(email);
            if (obj.idticket == 0)
            {
                return null;
            }
        ...
        private void DataPortal_Fetch(string email)
        {
            using (DatabaseManager ctx = new DatabaseManager())
            {
                ...
                cm.CommandText = @"SELECT t.idticket, t.TicketNo, t.TicketSubject, t.Requester, t.CallerPosistion, t.Priority, t.TicketOwner, t.TicketType, t.TicketStatus, t.TicketDescription, 
                                  t.Solution, t.Escalation, t.CreatedBy, t.CreatedDate, t.UpdatedBy, t.UpdatedDate, c.FirstName, c.LastName FROM ticket t
                                  JOIN customer c ON t.requester = c.email 
                                  WHERE Requester = @Requester";
                cm.Parameters.AddWithValue("@Requester", email);
                SafeDataReader dr = ctx.Read(cm);
                while (dr.Read())
                {
                    DataPortal_Fetch(dr);
